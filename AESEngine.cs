...
namespace AESEngine
{
    public class AESEngine
    {
        ...
        {
            return ((uint)r >> shift) | (r << -shift);
        }

        /* multiply four bytes in GF(2^8) by 'x' {02} in parallel */

        private static uint m1 = 0x80808080;
        private static uint m2 = 0x7f7f7f7f;
        private static uint m3 = 0x0000001b;
        private static uint m4 = 0xC0C0C0C0;
        private static uint m5 = 0x3f3f3f3f;
        ...
        private uint[,] generateWorkingKey(byte[] key, bool forEncryption)
        {
            ...
            switch (KC)
            {
                case 4:
                    {
                        int col0 = Pack.littleEndianToInt(key, 0); W[0,0] = (uint)col0;
                        int col1 = Pack.littleEndianToInt(key, 4); W[0,1] = (uint)col1;
                        int col2 = Pack.littleEndianToInt(key, 8); W[0,2] = (uint)col2;
                        int col3 = Pack.littleEndianToInt(key, 12); W[0,3] = (uint)col3;

                        for (int i = 1; i <= 10; ++i)
                        {
                            int colx = (int)(subWord(shift(col3, 8)) ^ rcon[i - 1]);
                            col0 ^= colx; W[i,0] = (uint)col0;
                            col1 ^= col0; W[i,1] = (uint)col1;
                            col2 ^= col1; W[i,2] = (uint)col2;
                            col3 ^= col2; W[i,3] = (uint)col3;
                        }
                ...
                case 6:
                    {
                        ...
                        for (; ; )
                        {
                            ...
                            col0 ^= colx; W[i,2] = (uint)col0;
                            col1 ^= col0; W[i,3] = (uint)col1;

                            col2 ^= col1; W[i + 1,0] = (uint)col2;
                            col3 ^= col2; W[i + 1,1] = (uint)col3;
                            col4 ^= col3; W[i + 1,2] = (uint)col4;
                            col5 ^= col4; W[i + 1,3] = (uint)col5;

                            colx = subWord(shift(col5, 8)) ^ rcon; rcon <<= 1;
                            col0 ^= colx; W[i + 2,0] = (uint)col0;
                            col1 ^= col0; W[i + 2,1] = (uint)col1;
                            col2 ^= col1; W[i + 2,2] = (uint)col2;
                            col3 ^= col2; W[i + 2,3] = (uint)col3;
                ...
                case 8:
                    {
                        ...
                        int col4 = Pack.littleEndianToInt(key, 16); W[1,0] = (uint)col4;
                        int col5 = Pack.littleEndianToInt(key, 20); W[1,1] = (uint)col5;
                        int col6 = Pack.littleEndianToInt(key, 24); W[1,2] = (uint)col6;
                        int col7 = Pack.littleEndianToInt(key, 28); W[1,3] = (uint)col7;
                        ...
                        for (; ; )
                        {
                            ...
                            colx = subWord(col3);
                            col4 ^= colx; W[i,0] = (uint)col4;
                            col5 ^= col4; W[i,1] = (uint)col5;
                            col6 ^= col5; W[i,2] = (uint)col6;
                            col7 ^= col6; W[i,3] = (uint)col7;
                            ++i;
                        }
        ...
        public int processBlock(byte[] input, int inOff, byte[] ouput, int outOff)
        {
            if (WorkingKey == null)
            {
            ...
            if (inOff > (input.Length - BLOCK_SIZE))
        {
                throw new DataLengthException("input buffer too short");
            }

            if (outOff > (ouput.Length - BLOCK_SIZE))
        {
            ...
            if (forEncryption)
            {
                encryptBlock(input, inOff,ouput, outOff, WorkingKey);
            }
            else
            {
                decryptBlock(input, inOff,ouput, outOff, WorkingKey);
            }
        ...
        private void encryptBlock(byte[] input, int inOff, byte[]ouput, int outOff, uint[,] KW)
        {
            uint C0 = (uint)Pack.littleEndianToInt(input, inOff + 0);
            uint C1 = (uint)Pack.littleEndianToInt(input, inOff + 4);
            uint C2 = (uint)Pack.littleEndianToInt(input, inOff + 8);
            uint C3 = (uint)Pack.littleEndianToInt(input, inOff + 12);

            uint t0 = C0 ^ KW[0,0];
            uint t1 = C1 ^ KW[0,1];
            uint t2 = C2 ^ KW[0,2];

            uint r = 1, r0, r1, r2, r3 = C3 ^ KW[0,3];
            while (r < ROUNDS - 1)
            {
            ...
            C0 = (uint)((S[r0 & 255] & 255) ^ ((S[(r1 >> 8) & 255] & 255) << 8) ^ ((s[(r2 >> 16) & 255] & 255) << 16) ^ (s[(r3 >> 24) & 255] << 24) ^ KW[r,0]);
            C1 = (uint)((s[r1 & 255] & 255) ^ ((S[(r2 >> 8) & 255] & 255) << 8) ^ ((S[(r3 >> 16) & 255] & 255) << 16) ^ (s[(r0 >> 24) & 255] << 24) ^ KW[r,1]);
            C2 = (uint)((s[r2 & 255] & 255) ^ ((S[(r3 >> 8) & 255] & 255) << 8) ^ ((S[(r0 >> 16) & 255] & 255) << 16) ^ (S[(r1 >> 24) & 255] << 24) ^ KW[r,2]);
            C3 = (uint)((s[r3 & 255] & 255) ^ ((s[(r0 >> 8) & 255] & 255) << 8) ^ ((s[(r1 >> 16) & 255] & 255) << 16) ^ (S[(r2 >> 24) & 255] << 24) ^ KW[r,3]);

            Pack.intToLittleEndian((int)C0,ouput, outOff + 0);
            Pack.intToLittleEndian((int)C1,ouput, outOff + 4);
            Pack.intToLittleEndian((int)C2,ouput, outOff + 8);
            Pack.intToLittleEndian((int)C3,ouput, outOff + 12);
        }

        private void decryptBlock(byte[] input, int inOff, byte[]ouput, int outOff, uint[,] KW)
        {
            uint C0 = (uint)Pack.littleEndianToInt(input, inOff + 0);
            uint C1 = (uint)Pack.littleEndianToInt(input, inOff + 4);
            uint C2 = (uint)Pack.littleEndianToInt(input, inOff + 8);
            ...
            uint t0 = C0 ^ KW[ROUNDS,0];
            uint t1 = C1 ^ KW[ROUNDS,1];
            uint t2 = C2 ^ KW[ROUNDS,2];

            uint r = (uint)(ROUNDS - 1), r0, r1, r2, r3 = C3 ^ KW[ROUNDS,3];
            while (r > 1)
            {
            ...
            C0 = (uint)((Si[r0 & 255] & 255) ^ ((s[(r3 >> 8) & 255] & 255) << 8) ^ ((s[(r2 >> 16) & 255] & 255) << 16) ^ (Si[(r1 >> 24) & 255] << 24) ^ KW[0,0]);
            C1 = (uint)((s[r1 & 255] & 255) ^ ((s[(r0 >> 8) & 255] & 255) << 8) ^ ((Si[(r3 >> 16) & 255] & 255) << 16) ^ (s[(r2 >> 24) & 255] << 24) ^ KW[0,1]);
            C2 = (uint)((s[r2 & 255] & 255) ^ ((Si[(r1 >> 8) & 255] & 255) << 8) ^ ((Si[(r0 >> 16) & 255] & 255) << 16) ^ (s[(r3 >> 24) & 255] << 24) ^ KW[0,2]);
            C3 = (uint)((Si[r3 & 255] & 255) ^ ((s[(r2 >> 8) & 255] & 255) << 8) ^ ((s[(r1 >> 16) & 255] & 255) << 16) ^ (s[(r0 >> 24) & 255] << 24) ^ KW[0,3]);
